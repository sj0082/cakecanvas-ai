...
- **Request**: `id, size_category_id, stylepack_id, user_text, parsed_slots(JSON), user_images[], contact(email/phone), consent_flags, access_token, created_at, payment_status, status`
  - `access_token`: 게스트 사용자 접근 토큰 (비인증 환경에서 본인 요청 조회용)
  - `status`: 요청 상태 (DRAFT, GENERATING, PROPOSALS_READY, PAYMENT_PENDING, DEPOSIT_PAID 등)
...
- `POST /functions/v1/create-request` → `{ id, access_token, ... }` (요청 생성)
- `GET /functions/v1/get-request?requestId={id}&accessToken={token}` → 요청 + 제안 조회

### 8.2 구현 필요 엔드포인트

- `POST /functions/v1/generate-proposals` → 3종 제안 생성 (비동기, Gemini 2.5 Flash Image 사용)
- `POST /functions/v1/evaluate-reality-rules` → 현실성 배지 생성
- `POST /functions/v1/select-proposal` → 선호안 선택 + 결제 링크
- `POST /functions/v1/payment-webhook` → Stripe 웹훅 처리 (Phase 3)
...
- 관리자 JWT/RBAC, CORS/CSP 화이트리스트([bakesbymarie.com](http://bakesbymarie.com/)).
...
- ✅ 기본 Edge Functions (create-request, get-request)
- ✅ 고객 여정 페이지 (Size/Style 선택, DesignDetails)
- ✅ 한국어 i18n 지원
- ⏳ 테스트 데이터 입력 대기
...
- **모니터링**: Supabase Dashboard + Edge Function Logs