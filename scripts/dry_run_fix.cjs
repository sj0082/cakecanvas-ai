    ...
    try {
        ...
        `, [name]);
            ...
            if (candidates.length > 1) {
                ...
                console.log(`  KEEP: ID ${toKeep.id} (Children: ${toKeep.childCount}, Created: ${toKeep.created_at})`);

                for (let i = 1; i < candidates.length; i++) {
                    const toDelete = candidates[i];
                    console.log(`  DELETE: ID ${toDelete.id} (Children: ${toDelete.childCount}, Created: ${toDelete.created_at})`);
                }
            } else {
